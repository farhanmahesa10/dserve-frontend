    ...
    <div className="bg-slate-50 pb-20">
      <div className="flex justify-between items-center ">
        <div className="px-2 w-full">
          <Link
            href={`/menu/${urlCode}`}
            className="mt-4  text-center flex items-center gap-2 clickable whitespace-nowrap"
          >
            <FaArrowLeftLong className="mt-[1px]" />{" "}
      ...
      <div className="w-full  grid grid-cols-1 md:grid-cols-6  px-2 ">
          ...
          {pesanan.length === 0 ? (
            ...
            <div className="mt-6  flex flex-col gap-8">
              {[...pesanan].reverse().map((item) => (
                <div
                  key={item.id_menu}
                  className="flex  gap-4 bg-white p-2 shadow-md"
                >
                  <div className="flex-shrink-0 flex-grow-0">
                    <img
                      src={`${process.env.NEXT_PUBLIC_PHOTOS}/${item.photo}`}
                      alt={item.title}
                      className="  aspect-video h-[100px]  object-cover rounded-[4px] md:rounded-bl-none md:rounded-br-none"
                    />
                  </div>
                  <div className="flex flex-col justify-between w-full">
                    <div className="">
                      <h2 className="font-semibold text-lg">{item.title}</h2>
                      <p className="text-sm text-slate-500 line-clamp-2 lg:line-clamp-5">
                        {item.details}
                      </p>
                    </div>
        ...
        <div className="md:col-span-2 shadow-md bg-white p-4 h-fit mt-6 sticky top-20">
          ...
          <div className="text-center mt-2">
            <Link
              href={`/menu/${urlCode}`}
              className="mt-4 text-yellow-700 text-center"
            >
              Back To Menu
            </Link>
          </div>
      ...
      {result?.success && (
        <div className="  bg-black bg-opacity-50 flex fixed top-0 w-screen h-screen justify-center items-center ">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center">
            <h2 className="text-xl font-bold text-start">{result.message}!</h2>
            <p className="mt-2 text-gray-600">
              Order has been successfully created, we will send your order to
              your room.
            </p>

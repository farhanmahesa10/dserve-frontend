...
export default function HeaderMenu(outletCode) {
  ...
  useEffect(() => {
    if (codeOutlet) {
      dispatch(fetchCategories(codeOutlet));
    }
  }, [dispatch, codeOutlet]);
  ...
  return (
    <div className=" fixed left-0 mt-16 w-full z-40   ">
      <div className="container ">
        <div className="flex items-center justify-center text-sm gap-4 px-4 py-1  ">
          <div className="flex gap-6 bg-yellow-700   px-4 rounded-bl-md rounded-br-md justify-center h-8">
            ...
            {categories &&
              categories.map((item) => (
                <button key={item.id} onClick={() => scrollToSection(item.type)} className="hover:text-gray-300 cursor-pointer font-semibold text-white capitalize transition-colors duration-300 ">
                  {item.type}
                </button>
              ))}
          </div>

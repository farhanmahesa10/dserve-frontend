  ...
  return (
    <div className="bg-white shadow-md rounded-md  p-2 flex gap-2 md:flex-col w-full">
      <div className="flex-shrink-0">
        <img
          src={`${process.env.NEXT_PUBLIC_PHOTOS}/${menu.photo}`}
          alt={menu.title}
          className="md:w-full w-32 md:h-[200px] h-[100px] object-cover rounded-[4px] md:rounded-bl-none md:rounded-br-none"
        />
      </div>
      <div className="flex justify-between flex-col">
        <div className=" ">
          <div>
            <h1 className="capitalize text-slate-800 font-semibold text-sm sm:text-lg md:text-base ">
              {menu.title}
            </h1>
            <p
              className="text-slate-700 line-clamp-2 text-xs md:py-1   "
              title={menu.details}
            >
              {menu.details}
            </p>
  ...
  return (
    <div className="grid grid-cols-2 gap-2 md:grid-cols-1 md:flex-row bg-white shadow rounded-xl border-2 overflow-hidden w-[300px] sm:w-full max-w-[700px] p-2 md:p-4">
      <img
        src={`${process.env.NEXT_PUBLIC_PHOTOS}/${menu.photo}`}
        alt={menu.title}
        className="md:w-full w-32 md:h-[200px] h-[100px] object-cover rounded-lg"
      />
      ...
      <div className="flex flex-col  justify-between  flex-1">
        ...
        <div className="flex items-center gap-2  justify-between mt-3">
          <p className="text-slate-700 text-xs sm:text-sm  md:hidden">
            {formatToRupiah(menu.price)}
          </p>
          <CounterButton id_menu={menu.id} onIncrement={onIncrement} />
        </div>
